...
void netCGI_ProcessData (uint8_t code, const char *data, uint32_t len) {
  ...
  do {
    ...
    if (var[0] != 0) {
      ...
      }else if (strcmp (var, "ctrl=Tension") == 0){
				formato=0;
				osThreadFlagsSet (TID_Display, 0x01U);
      }else if (strcmp (var, "ctrl=Frecuencia") == 0){
				formato=1;
				osThreadFlagsSet (TID_Display, 0x01U);
      }
		}
  ...
  switch (env[0]) {
    ...
    case 'f':
				switch (env[2]) {
					case '1':
						if (formato)
							sprintf(mytemp, "%0.3fHz %0.1f1ºC", medidaf, temperatura); 
						else
							sprintf(mytemp, "%0.3fV %0.1f1ºC", medidav, temperatura); 
						len = (uint32_t)sprintf (buf, &env[4], mytemp);
					break;
					case '2':
						// Select Control
						len = (uint32_t)sprintf (buf, &env[4], formato ?     ""     : "selected",
                                               formato ? "selected" :    ""     );
					break;
				}
				break;
    case 'g':
      ...
      switch (env[2]) {
        case '1':
          //adv = medida_adc(5);
					//actualizar el estado del rbg Comparar eel valor y actuar
         // getAdcSetRGB(adv);
    ...
    case 'y':
     ...
     // len = (uint32_t)sprintf (buf, "<checkbox><id>button%c</id><on>%s</on></checkbox>",
      ...
      break;
			case 'q':
					if (formato)
						sprintf(mytemp, "%0.3fHz %0.1fC", medidaf, temperatura); 
					else
						sprintf(mytemp, "%0.3fV %0.1fC", medidav, temperatura); 
          len = (uint32_t)sprintf (buf, &env[4], mytemp);
